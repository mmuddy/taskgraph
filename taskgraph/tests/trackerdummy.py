...
class TrackerDummy(TrackerInterface):
    ...
    def __init__(self):
        ...
        self.assignees = TrackerDummy.create_kw_args_set(['name', 'identifier'],
                                                         [['dev1', 1], ['dev2', 2], ['dev3', 3],
                                                          ['tstr1', 4], ['tstr2', 5],
                                                          ['mngr1', 6], ['mngr2', 7]])
        ...
        self.task_categories = TrackerDummy.create_kw_args_set(['name', 'identifier'],
                                                               [['bug', 1], ['feature', 2], ['task', 3], ['question', 4]])

        self.task_states = TrackerDummy.create_kw_args_set(['name', 'identifier'],
                                                           [['done', 1], ['assigned', 2],
                                                            ['closed', 3], ['in progress', 4]])
    ...
    def get_tasks(self, _):
        ...
        for ind in range(15):
            ...
            new_task.assignee = (self.assignees[randint(0, len(self.assignees) - 1)]['name'], 1)
            new_task.milestone = self.milestones[randint(0, len(self.milestones) - 1)]['name']
            new_task.category = (self.task_categories[randint(0, len(self.task_categories) - 1)]['name'], 1)
            new_task.state = (self.task_states[randint(0, len(self.task_states) - 1)]['name'], 1)
